\section{Analisi Quantitativa}\label{sec:analisi-quantitativa}
...
Per quanto concerne la Intel RealSense D435i, è stato necessario definire una strategia preliminare basata sulla logica di funzionamento del sensore. Il calcolo dell'NDVI richiede la fusione di due bande spettrali: il canale Rosso (proveniente dal sensore RGB) e il canale NIR (proveniente dal sensore stereo Infrarosso sinistro). Tramite la libreria \texttt{pyrealsense2} è possibile allineare RGB e Infrarosso utilizzando la Depth come riferimento; poiché la mappa di profondità è generata nativamente dal punto di vista del sensore Infrarosso sinistro, l'allineamento del colore alla Depth equivale geometricamente all'allineamento rispetto al sensore IR. È importante precisare che, pur effettuando l'allineamento dell'intero frame RGB, nella pipeline NDVI non viene utilizzata tutta l'informazione colore: viene estratta esclusivamente la componente rossa, in quanto unica banda necessaria insieme al NIR per il calcolo dell'indice.

Tuttavia, l'utilizzo diretto dei flussi allineati in fase di calibrazione si è rivelato problematico. La scacchiera non si è rivelata una buona scelta per il calcolo della disparità (e quindi della profondità) perché il suo pattern ripetitivo e ad alto contrasto non permette una stima uniforme e puntuale della mappa di profondità (in alcune regioni non è possibile rilevare la profondità, per cui la mappa risulta come "bucata"). Poiché l'operazione di allineamento propaga questi buchi anche agli stream RGB e IR, l'utilizzo delle immagini allineate avrebbe compromesso la rilevazione degli angoli della scacchiera. Di conseguenza, per la calibrazione del sistema stereo si è deciso di utilizzare l'immagine infrarossa originale, bypassando l'allineamento alla depth in questa fase. Inoltre, per la RealSense non sono stati ricalcolati i parametri intrinseci, ma si è scelto di utilizzare i valori di calibrazione di fabbrica forniti tramite la libreria \texttt{pyrealsense2}, garantendo così l'alta affidabilità prevista dal produttore.

Per la Mapir Survey 3, prima di procedere alla calibrazione degli intrinseci, è stato necessario un pre-processamento delle immagini. La notevole disparità di risoluzione nativa tra la Mapir (4000x3000 pixel) e la RealSense (1280x720 pixel) rendeva instabile la stima dei parametri estrinseci. Le immagini acquisite dalla Mapir sono state ridimensionate per eguagliare la larghezza della RealSense, adattando successivamente l'altezza mediante ritaglio per passare da 960 a 720 pixel. Questa operazione di normalizzazione delle dimensioni garantisce che ogni pixel delle due mappe rappresenti un'area fisica comparabile della vegetazione e rende il confronto quantitativo più coerente. Inoltre, queste immagini sono state convertite in scala di grigi. A partire da queste immagini elaborate, sono stati calcolati i parametri intrinseci specifici per la Mapir Survey 3.
...
Per la Mapir, invece, la generazione della maschera è avvenuta mediante una sogliatura (thresholding) sui valori dell'NDVI stesso. Essendo l'interesse limitato alla sola biomassa vegetale, è stato impostato empiricamente un valore di soglia minima (0.1) per filtrare il background e tutto ciò che si trovasse al di sotto di tale livello. Come osservabile in Fig. \ref{fig:ndvi-masking}, questo filtraggio preserva la continuità della chioma senza creare lacune, in quanto i valori di NDVI sulla vegetazione sana si mantengono consistentemente sopra la soglia.
...
A livello globale, l'errore medio assoluto tra i due sensori si attesta a 0,257, con un RMSE di 0,284. La correlazione di Pearson, pari a 0,258, indica un legame debole tra le due serie di misure. La distribuzione spaziale delle diversità tra le risposte dei due sensori è mostrata in Fig. \ref{fig:ndvi-error-heatmap}, da cui si evince come la divergenza tra i due sensori non sia uniforme, ma distribuita in modo eterogeneo sulla superficie fogliare, confermando che il sensore RealSense non può sostituire la fotocamera Mapir per applicazioni che richiedano accuratezza radiometrica locale senza una calibrazione specifica.
